...
exports.itemSeed=async(req,res,next)=>{
 ...
 let [categoryResult] = await connection.query("SELECT category_id FROM categories",[])
  let categories= []
...
for(let i in bags){
  items.push(
     {
    item : bags[i],
    category : categories[faker.number.int({max:categories.length-1,min:0})],
    item_price : `${faker.number.int({min:100,max:100000})}.${faker.number.int({min:0,max:99})}`,
